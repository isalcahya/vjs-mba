...
import './SettingSubOptionItem';
import './SettingSubOptionTitle';
import './SettingMenuItem';

const SettingMenuItem       = videojs.getComponent('SettingMenuItem');
const SettingSubOptionTitle = videojs.getComponent('SettingSubOptionTitle');
  ...
  setEntries(entries_ = [], selectedIndex) {
    ...
    this.subMenuItems = [
      ...
      ...this.entries.map(({ label, value }, index) => {
        let insertSub = new SubOptionItem(this.player_, { index, label, value, parent: this, menu: this.menu });
        return insertSub;
      })
    ];
  }
